...
| Legion         | Replaces the normal distribution with repeated Legion copies and a small good minority. An active Legion setup allocates the inverted distribution before offers in every presentation mode: every evil slot receives three Legion wheels that all resolve to Legion with no mulligan, while good slots and mulligans contain only Townsfolk or Outsiders and never Legion. Standard setup can discover Legion until the first `Legion good count` players have completed good drafts: at ten players, Legion may therefore activate on drafts one through four, with the fourth player becoming the first evil player after three good players are committed. Each eligible player rolls one script Demon uniformly. That Demon appears once in a randomized position among the normal three choices; the other choices and the mulligan are restricted to Townsfolk and Outsiders. Rolling Legion leaves the persisted setup and hidden type plans Standard while the player considers the offer; only choosing Legion switches the setup, clears incompatible Standard reservations, and replans the remaining players to the inverted distribution. Choosing a good option or taking the good-only mulligan leaves the setup Standard. Rolling any non-Legion Demon, such as Lleech, visibly offers that Demon and permanently eliminates Legion from every later wheel and mulligan, whether or not the player chooses it. Finishing the fourth draft without activation also makes Legion unavailable. |
...
3. Storyteller selects any unresolved player. The engine generates and persists
   ...
   Switching to another unresolved player discards the prior unrevealed preview,
   so draft state contains completed entries plus at most one active offer.
...
- [x] Keep only the active unresolved preview when the Storyteller switches
      player pills so stale hidden-role warnings cannot accumulate.
