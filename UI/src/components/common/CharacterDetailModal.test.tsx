...
const fullCharacter: CharacterDef = {
  ...
  otherNights: {
    order: 10,
    helpText:
      'The Fortune Teller points to two players. Give the thumbs up or thumbs down.',
    subActions: [
      {
        id: 'ft-on-1',
        description: 'The Fortune Teller points to two players.',
...
describe('CharacterDetailModal', () => {
  ...
  it('renders nothing when open is false (dialog hidden)', () => {
    render(
      <CharacterDetailModal open={false} character={fullCharacter} onClose={vi.fn()} />,
    );
  ...
  it('renders modal with character name when open', () => {
    render(
      <CharacterDetailModal open={true} character={fullCharacter} onClose={vi.fn()} />,
    );
  ...
  it('shows the "Detailed Rules" heading and detailed text when available', () => {
    ...
    expect(
      screen.getByText(/The Fortune Teller detects Demons/),
    ).toBeInTheDocument();
  });

  it('shows fallback text when abilityDetailed is not available', () => {
    render(
      <CharacterDetailModal open={true} character={minimalCharacter} onClose={vi.fn()} />,
    );
    expect(screen.getByText('Detailed Rules')).toBeInTheDocument();
  ...
  it('calls onClose when close button is clicked', async () => {
    const onClose = vi.fn();
    render(
      <CharacterDetailModal open={true} character={fullCharacter} onClose={onClose} />,
    );
  ...
  it('handles Evil-aligned character correctly', () => {
    render(
      <CharacterDetailModal open={true} character={evilCharacter} onClose={vi.fn()} />,
    );
